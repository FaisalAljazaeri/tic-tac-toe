...
function onConnection(socket) {
    ...
    if (waitingPlayer) {
        ...
        waitingPlayer.emit('chat', 'Opponent joind.');
        socket.emit('chat', 'You are ready to play');
        ...
        socket.emit('chat', 'waiting for an opponent...');
    }
}

function connectChat(socket1, socket2) {
    // register messaging between the two players.
    socket1.on('chat', msg => socket1.emit('chat', msg));
    socket2.on('chat', msg => socket2.emit('chat', msg));

    socket1.on('chat', msg => socket2.emit('chat', msg));
    socket2.on('chat', msg => socket1.emit('chat', msg));
}
